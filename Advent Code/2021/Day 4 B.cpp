...
public:
	...
	void insertar(int val, int x, int y){
		t[5*y+x]=val;
	}
	bool marcar(int val){
		m++;
...
int main(){
	...
	int last_value=-1, cant=0;
	set<int> comp;
	for(size_t i=0;i<valores.size();++i){
		auto it = m.find(valores[i]);
		if(it!=m.end()){
			for(auto it1=(it->second).begin();it1!=(it->second).end();++it1){
				if(comp.find(*it1)==comp.end() && v[*it1].marcar(valores[i])){
					cant++;
					comp.insert(*it1);
					if(cant==v.size()){sum=v[*it1].suma();last_value=valores[i];break;}
				}
			}
			if(cant==v.size())break;
			(it->second).clear();
		}
	}
